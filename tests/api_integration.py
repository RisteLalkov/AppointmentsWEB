...
with tempfile.TemporaryDirectory(prefix='careline-api-') as temporary:
    ...
    try:
        ...
        other_doctor = Client(api1).demo('user-d02')
        ...
        check(doctor.request('/api/schedule/d02', schedule)[0] == 403, 'Doctor cannot edit another schedule')
        ...
        admin.ok('/api/accounts/user-d02/credentials', {'email': 'doctor@integration.test', 'password': password})
        check(Client(api2).login('doctor@integration.test').ok('/api/auth/me')['actor']['doctorId'] == 'd02', 'Doctor password credentials preserve profile link')
        check(anon.request('/api/auth/demo-login', {'userId': 'user-d02'})[0] == 404, 'Provisioned doctor removed from anonymous demo login')
