...
public sealed class DataController(IServiceProvider services, DemoIdentity identity, SchedulingClock clock, BackendSettings settings, ApiClient api) : Controller
{
    ...
    [HttpGet("slots")] public Task<IActionResult> Slots(string doctorId, DateOnly date, string? excludeId) => Execute($"slots?doctorId={Uri.EscapeDataString(doctorId)}&date={date:yyyy-MM-dd}&excludeId={Uri.EscapeDataString(excludeId ?? "")}", null, () => Demo.Availability(identity.Actor, doctorId, date, excludeId));
    [HttpGet("exceptions")] public Task<IActionResult> Exceptions(string doctorId) => Execute("exceptions?doctorId=" + Uri.EscapeDataString(doctorId), null, () => Demo.Exceptions(identity.Actor, doctorId));
    ...
    [HttpPost("schedule/{doctorId}")] public Task<IActionResult> Schedule(string doctorId, [FromBody] ScheduleInput input) => Execute("schedule/" + Uri.EscapeDataString(doctorId), input, () => { Demo.ReplaceSchedule(identity.Actor, doctorId, input.Periods, input.DurationMinutes); return new { saved = true }; });
    [HttpPost("exceptions/{doctorId}")] public Task<IActionResult> Exception(string doctorId, [FromBody] ExceptionInput input) => Execute("exceptions/" + Uri.EscapeDataString(doctorId), input, () => Demo.AddException(identity.Actor, doctorId, input.Date, input.Start, input.End, input.IsAvailable, input.Reason));
